...
static int element_Compare(const element * const left_Element
  , const element * const right_Element){
        ...
        // If we got here then both items have the same value so then sort based
 on
        // key.
...
static void generate_Frequencies_For_Desired_Length_Oligonucleotides(
  ...
  , const intnative_t desired_Length_For_Oligonucleotides, char * const output){
        ...
        uint64_t key=0;
        const uint64_t mask=((uint64_t)1<<2*desired_Length_For_Oligonucleotides)
-1;

        // For the first several nucleotides we only need to append them to key
in
        // preparation for the insertion of complete oligonucleotides to hash_Ta
ble.
        ...
        // desired_Length_For_Oligonucleotides to hash_Table and update the coun
t
        ...
        for(intnative_t i=desired_Length_For_Oligonucleotides-1
          ; i<polynucleotide_Length; i++){
                ...
                // If the element_Was_Unused, then initialize the count to 1, ot
herwise
                // increment the count.
...
--){
                        oligonucleotide[j]=nucleotide_For_Code(elements_Array[i]
.key);
...
herwise
        ...
        snprintf(output, MAXIMUM_OUTPUT_LENGTH, "%ju\t%s", count, oligonucleotid
e);
...
int main(){
        ...
        while(fgets(buffer, sizeof(buffer), stdin) && buffer[0]!=ʼ>ʼ){
                ...
                // Make sure we still have enough memory allocated for any poten
tial
                // nucleotides in the next line.
                if(polynucleotide_Capacity-polynucleotide_Length<sizeof(buffer))
                        polynucleotide=realloc(polynucleotide, polynucleotide_Ca
pacity*=2);
        }
        ...
        #pragma omp parallel sections
        {
                ...
                #pragma omp section
                generate_Frequencies_For_Desired_Length_Oligonucleotides(polynuc
leotide
