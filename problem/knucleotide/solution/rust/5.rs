...
// Rust #3 modified to use same get_seq as Rust #4
...
impl Item {
    fn print(&self, freq: &Map) {
        match *self {
            Freq(frame) => {
                let mut v: Vec<_> = freq.iter().map(|(&code, &count)| (count, co
de)).collect();
                v.sort();
                let total = v.iter().map(|&(count, _)| count).sum::<u32>() as f3
2;
                for &(count, key) in v.iter().rev() {
                    println!("{} {:.3}", key.to_string(frame), (count as f32 * 1
00.) / total);
                }
                println!("");
            }
...
fn get_seq<R: std::io::BufRead>(mut r: R, key: &[u8]) -> Vec<u8> {
    ...
    loop {
        line.clear();
        match r.read_until(b'\n', &mut line) {
            Ok(b) if b > 0 => res.extend(line[..line.len()-1].iter().cloned().ma
p(Code::encode)),
            _ => break,
        }
    }
