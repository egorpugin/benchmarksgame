...
,
        ...
        for (int fragmentLength : fragmentLengths) {
            for (int index = 0; index < fragmentLength; index++) {
                int offset = index;
                tasks.add(() -> createFragmentMap(sequence, offset, fragmentLeng
th));
            }
        }
        return tasks;
    ...
    static Result createFragmentMap(byte[] sequence, int offset, int fragmentLen
gth) {
...
());
        frequencies.map.forEach((key, cnt) -> freq.add(new SimpleEntry<>(keyToSt
ring(key,
...
ment)
            throws Exception {
        byte[] key = toCodes(nucleotideFragment.getBytes(StandardCharsets.ISO_88
59_1),
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<Future<Result>> futures = pool.invokeAll(createFragmentTasks(sequen
ce,
                fragmentLengths));
        pool.shutdown();
