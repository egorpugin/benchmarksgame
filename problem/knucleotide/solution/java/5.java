...
public class knucleotide extends ForkJoinTask {
    ...
    static final ForkJoinPool pool = new ForkJoinPool(Runtime.getRuntime().avail
ableProcessors());
    ...
    static final String[] SPICIFIC_SEQUENCES = new String[]{"GGT", "GGTA", "GGTA
TT", "GGTATTTTAATT", "GGTATTTTAATTTATAGT"};
...
outer:
    ...
    static void printSequence(List<Entry<Key, Value>> sequence) {
        ...
        Collections.sort(sequence, new Comparator<Entry<Key, Value>>() {
            ...
            @Override
            public int compare(Entry<Key, Value> entry1, Entry<Key, Value> entry
2) {
                if (entry2.getValue().count != entry1.getValue().count) {
                    return entry2.getValue().count - entry1.getValue().count;
                }
                return entry1.getKey().toString().compareTo(entry2.getKey().toSt
ring());
            }
        });
        ...
        for (Entry<Key, Value> entry : sequence) {
            System.out.format("%s %.3f\n", entry.getKey(), entry.getValue().coun
t * 100f / sum);
        }
        System.out.println();
    }
