...
public class Program
{
    ...
    private static void WriteFrequencies(Dictionary<ulong, Wrapper> freq, int bu
flen, int fragmentLength)
    {

        double percent = 100.0 / (buflen - fragmentLength + 1);
        foreach (var line in (from k in freq.Keys
                              orderby freq[k].V descending
                              select string.Format("{0} {1:f3}", PrintKey(k, fra
gmentLength),
    ...
    private static void WriteCount(Dictionary<ulong, Wrapper> dictionary, string
 fragment)
    {
        ulong key = 0;
    ...
    private static Dictionary<ulong, Wrapper> CountFrequency(byte[] buffer, int
fragmentLength)
    {
    ...
    private static byte[] GetBytesForThirdSequence()
    {
        ...
        int amountRead, threebuflen, indexOfFirstByteInThreeSequence, indexOfGre
aterThan, threepos, tocopy;
        amountRead = threebuflen = indexOfFirstByteInThreeSequence = indexOfGrea
terThan = threepos = tocopy = 0;
...
> 0)
        {
            ...
            if (threeFound)
            {
                ...
                indexOfFirstByteInThreeSequence = Array.IndexOf<byte>(buffer, 10
, indexOfGreaterThan) + 1;
                tocopy = amountRead - indexOfFirstByteInThreeSequence;
                if (amountRead < buffersize)
                    tocopy -= 1;
                Buffer.BlockCopy(buffer, indexOfFirstByteInThreeSequence, threeb
uffer, 0, tocopy);
                buffer = null;
            }
            else
