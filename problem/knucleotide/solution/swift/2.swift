...
let queue = DispatchQueue(label: "hashQueue", attributes: DispatchQueue.Attribut
es.concurrent)
...
func readInputAndRewrite() -> [Int8] {
    ...
    var seq = [Int8]()
    while let res = fgets(buf, 100, stdin) {
        seq.append(contentsOf: UnsafeMutableBufferPointer(start: res, count: lin
ewidth))
    }
...
for k in hash.keys.sorted(by: { hash[$1]! < hash[$0]! }) {
    print("\(i2c[k]!) \(String(format: "%.3f", 100.0*Double(hash[k]!)/Double(tot
al)))")
}
print()
...
for k in hash2.keys.sorted(by: { hash2[$1]! < hash2[$0]! }) {
    print("\(decode(k, 2)) \(String(format: "%.3f", 100.0*Double(hash2[k]!)/Doub
le(total2)))")
}
print()
