...
func count32(dna []byte, length int) map[uint32]uint32 {
    ...
    for index := uint(length) - 1; index < uint(len(dna)); index++ { // Use an u
int as index to allow the compiler to remove a bounds check on the dna slice
        key = key<<2&mask | uint32(dna[index])
        counts[key]++ // No need to check if key is already present in map since
 Go returns the zero value of the map value if the key is not present
    }
    return counts
}
