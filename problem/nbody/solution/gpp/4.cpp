...
void advance(int nbodies, struct planet * bodies, const v2df& dtdt)
{
  ...
  for (i = 0; i < nbodies; i++) {
    ...
    for (j = i + 1; j < nbodies; j++) {
      ...
      v2df tsquare = __builtin_ia32_haddpd(dxdy*dxdy,dz00*dz00);    // dx*dx+dy*
dy | dz*dz
      v2df distance2 = __builtin_ia32_haddpd(tsquare,tsquare);      // dx*dx+dy*
dy+dz*dz | dx*dx+dy*dy+dz*dz
...
double energy(int nbodies, struct planet * bodies)
{
  ...
  for (i = 0; i < nbodies; i++) {
    struct planet * b = &(bodies[i]);
    v2df sq = b->massmass * __builtin_ia32_haddpd(b->vxvy*b->vxvy, b->vz00*b->vz
00);  // b->mass*(vx*vx + vy*vy) | b->mass*(vz*vz + 0*0)
    ...
    for (j = i + 1; j < nbodies; j++) {
      ...
      v2df dxdy = b->xy - b2->xy;
      v2df dz00 = b->z0 - b2->z0;
      v2df distance = __builtin_ia32_haddpd(dxdy*dxdy, dz00*dz00);  // b->mass*(
vx*vx + vy*vy) | b->mass*(vz*vz + 0*0)
