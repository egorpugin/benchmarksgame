    ...
    fn advance(&mut self, dt: f64) {
        ...
        for (index, &Pair(i,j)) in self.pairs.iter().enumerate() {
                ...
                for index in 0..N_PAIRS {
                        self.distances_square[index] = self.offsets[index].squar
ed_norm();
                        self.distances[index] = self.distances_square[index].sqr
t();
                }
                for index in 0..N_PAIRS {
                        self.mags[index] = dt / (self.distances_square[index] *
self.distances[index]);
                }
        for (index, &Pair(i,j)) in self.pairs.iter().enumerate() {
                    planets[i].vel -= self.offsets[index] * planets[j].mass * se
lf.mags[index];
                    planets[j].vel += self.offsets[index] * planets[i].mass * se
lf.mags[index];
                }
