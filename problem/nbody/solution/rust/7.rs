...
/// A named type declaration corresponding to the anonymous type declaration in
`gcc #4`:
/// ```
/// static struct {
///   double dx[3], fill;
...
fn bodies_energy(bodies: &[Body; N_BODIES]) -> f64 {
    ...
    for i in 0..bodies.len() {
        ...
        for j in (i + 1)..bodies.len() {
            ...
            let distance = ((dx[0] * dx[0]) + (dx[1] * dx[1]) + (dx[2] * dx[2]))
.sqrt();
...
///
/// This is corresponding to the anonymous type-declaration being used in `gcc #
4`
/// ```
...
impl BodiesAdvance {
    ...
    /// Calculating advance of bodies within time dt, using the buffers `r` and
`mag`
    #[inline]
    pub fn bodies_advance(&mut self, bodies: &mut [Body; N_BODIES], dt: f64) {
        ...
        for i_2 in 0..(N / 2) {
            ...
            // ```
            let mut distance = unsafe { _mm_cvtps_pd(_mm_rsqrt_ps(_mm_cvtpd_ps(d
squared))) };

            // repeat 2 times
            for _ in 0..2 {
                // In `gcc #4` corresponding to implicit call of _m128f operatio
ns
                // ```
                ...
                distance = unsafe {
                    _mm_sub_pd(
                        ...
                        _mm_mul_pd(
                            _mm_mul_pd(_mm_mul_pd(_mm_set1_pd(0.5), dsquared), d
istance),
            ...
            // ```
            let dmag: __m128d =
                unsafe { _mm_mul_pd(_mm_div_pd(_mm_set1_pd(dt), dsquared), dista
nce) };
        ...
        for i in 0..(bodies.len() - 1) {
            for j in (i + 1)..bodies.len() {
                for m in 0..3 {
                    bodies[i].v[m] -= (self.r[k].dx[m] * bodies[j].mass) * (self
.mag.0)[k];

                    bodies[j].v[m] += (self.r[k].dx[m] * bodies[i].mass) * (self
.mag.0)[k];
                }
                k += 1;
            }
