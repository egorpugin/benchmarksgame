...
procedure Pidigits is
  ...
  package GMP_Mini is
        Print_String : constant String := ASCII.HT & ":%d" & ASCII.LF & ASCII.NU
L;
    ...
    procedure mpz_init_set_ui (Dest: out mpz_t; Src: in Interfaces.C.unsigned_lo
ng) with Inline=>True;
    pragma Import(C, mpz_init_set_ui, "__gmpz_init_set_ui");

    procedure mpz_mul_ui (Dest: out mpz_t; Src1: in mpz_t; Src2: in Interfaces.C
.unsigned_long) with Inline=>True;
    ...
    procedure mpz_addmul_ui (Dest: out mpz_t; Src1: in mpz_t; Src2: in Interface
s.C.unsigned) with Inline=>True;
    pragma Import(C, mpz_addmul_ui, "__gmpz_addmul_ui");

    procedure mpz_submul_ui (Dest: out mpz_t; Src1: in mpz_t; Src2: in Interface
s.C.unsigned) with Inline=>True;
    pragma Import(C, mpz_submul_ui, "__gmpz_submul_ui");

    procedure mpz_tdiv_q (Dest: out mpz_t; Src1, Src2: in mpz_t) with Inline=>Tr
ue;
    pragma Import(C, mpz_tdiv_q, "__gmpz_tdiv_q");

    function mpz_get_ui (Src: in mpz_t) return Interfaces.C.unsigned_long with I
nline=>True;
    pragma Import(C, mpz_get_ui, "__gmpz_get_ui");

    function mpz_cmp (Src: in mpz_t; Ref: in mpz_t) return Interfaces.C.int with
 Inline=>True;
    ...
    procedure Put_Line (Format : in String; Data : in Interfaces.C.unsigned) wit
h Inline=>True;
  ...
  function Extract(X: Interfaces.C.Unsigned_long) return Interfaces.C.unsigned w
ith Inline=>True;
  function Extract(X: Interfaces.C.Unsigned_long) return Interfaces.C.unsigned i
s
  begin
...
begin
  ...
  declare
    ...
    Space   : constant Interfaces.C.unsigned := 32;
  begin
    <<Work_Loop>>
    while I < N loop
      K := K + 1;
