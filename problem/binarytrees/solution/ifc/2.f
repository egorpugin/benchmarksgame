...
module apr
  ...
  interface
    ...
    integer(c_int) function apr_pool_create_unmanaged_ex(newpool, abort_fn, allo
cator) bind(C)
      import
...
program main
  ...
  write(*,"(2(a,i0))") "stretch tree of depth ",stretch_depth, t//" check: ",  n
ode_check(curr)
  ...
  !$omp parallel do private(store, a, b, c, i, iterations, tmp) schedule(dynamic
,1)
  do  d = min_depth, max_depth, 2
      ...
      write(outputstr(d),"(2(i0,a),i0)") iterations,t//" trees of depth ", d ,t/
/" check: ", c
  end do
  !$omp end parallel do
  ...
  write(*,"(2(a,i0))") "long lived tree of depth ", max_depth ,t//" check: ", no
de_check(long_lived_tree)
end program
