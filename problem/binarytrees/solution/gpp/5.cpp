...
int run_parallel(unsigned depth, int iterations, unsigned int workers = std::thr
ead::hardware_concurrency())
{
...
{
    const int max_depth     = std::max(MIN_DEPTH + 2, (argc == 2 ? atoi(argv[1])
 : 10));
    ...
    for (int d = MIN_DEPTH; d <= max_depth; d += 2)
    {
        ...
        std::cout << iterations << "\t trees of depth " << d << "\t check: " <<
c << "\n";
    }
