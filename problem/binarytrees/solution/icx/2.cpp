...
static tree_node * create_Tree(const intnative_t tree_Depth,
  apr_pool_t * const memory_Pool){
        ...
        // If tree_Depth is one or more then recursively call create_Tree() in o
rder
...
static intnative_t compute_Tree_Checksum(const tree_node * const root_Node){
        // If there are subtrees then recursively call compute_Tree_Checksum() o
n
        ...
        // If the function gets here then this is a single node tree so just ret
urn
        // 1 as the checksum.
        return 1;
}
...
int main(int argc, char *argv[]){
        // Set minimum_Tree_Depth to 4 and maximum_Tree_Depth to the maximum of
what
...
.
        ...
        // Create a memory_Pool which will be used for storing both the stretch_
Tree
...
less
        ...
        // that most of the memory that was already allocated for the stretch_Tr
ee
        // can be reused for the upcoming long_Lived_Tree work rather than havin
g
        // APR allocate more memory for memory pools. Unfortunately since the
        // long_Lived_Tree is about half the size of the stretch_Tree, this ends
 up
        // wasting about half the memory that was being used by the stretch_Tree
.
        // APR subpools could be used to use that otherwise wasted memory for th
e
...
ibly
        ...
        // The long_Lived_Tree will be created in just a little bit simultaneous
ly
        // (assuming OpenMP was enabled and the program is running on a multi-
        // processor system) while the rest of the trees are also being processe
d.
        // long_Lived_Tree will store the reference to it which will remain vali
d
        ...
        #pragma omp parallel
        {
                ...
                // maximum_Tree_Depth in the memory_Pool which was already previ
ously
                // used for the stretch_Tree, compute the long_Lived_Tree_Checks
um, and
                // then just leave the long_Lived_Tree alone for a while while t
he rest
                // of the binary trees finish processing (which should have
                // simultaneously been started to be processed by any other avai
lable
                // threads).
                #pragma omp single nowait
                {
                        long_Lived_Tree=create_Tree(maximum_Tree_Depth, memory_P
ool);
                        long_Lived_Tree_Checksum=compute_Tree_Checksum(long_Live
d_Tree);
                }
                ...
                for(intnative_t tree_Depth=minimum_Tree_Depth;
                  tree_Depth<=maximum_Tree_Depth; tree_Depth+=2){
                        ...
                        // Create a bunch of binary trees of depth tree_Depth, c
ompute their
                        // checksums, and add the checksums to the total_Trees_C
hecksum.
                        ...
                        // Record the total_Trees_Checksum for the trees of dept
h
                        // tree_Depth.
