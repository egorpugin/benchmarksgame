...
public class regexredux {
  ...
  public static void main(String[] args) throws IOException {
    ...
    final Map<String, Long> results = variants.parallelStream()
                                              .map(variant -> counts.apply(varia
nt, sequence))
                                              .collect(toMap(Map.Entry::getKey,
Map.Entry::getValue));

    variants.forEach(variant -> System.out.println(variant + " " + results.get(v
ariant)));

    final Map<String, String> replacements = new LinkedHashMap<>();//Only works
with LinkedHashMap
    {
    ...
    for (Map.Entry<String, String> entry : replacements.entrySet()) {
        buf = Pattern.compile(entry.getKey()).matcher(buf).replaceAll(entry.getV
alue());
    }
