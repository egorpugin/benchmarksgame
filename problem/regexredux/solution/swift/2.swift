...
for n in 0..<variants.count {
  counts[n] = (variants[n], regex(variants[n]).numberOfMatches(in: sequence, opt
ions: [], range: NSRange(location: 0, length: codeLength)))
}
...
for (re, replacement) in replacements {
  sequence = re.stringByReplacingMatches(in: sequence, options: [], range: NSRan
ge(location: 0, length: sequence.utf16.count), withTemplate: replacement)
}
