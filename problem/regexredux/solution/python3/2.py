...
def process_Task(worker_Pipe, sequences, sequences_Length):
        ...
        while True:
                ...
                # If there is no task to do, then just exit the loop and let thi
s
                # subprocess exit.
                if task is None:
                        break

                # If the task contains a tuple and the first element is an int,
then
                ...
                # index_For_Pattern_To_Count in count_Info[].
                        ...
                        match_Data=PCRE2.pcre2_match_data_create_8(c_uint32(1),
None)
                        ...
                        # Compile the pattern and also enable JIT compilation to
 make
                        # matching faster.
                        regex=PCRE2.pcre2_compile_8(pattern_To_Count,
                          c_size_t(len(pattern_To_Count)), c_uint32(0), byref(c_
int()),
                          byref(c_size_t()), None)
                        PCRE2.pcre2_jit_compile_8(regex, c_uint32(0x1))

                        # Count how many matches there are for pattern in sequen
ces.
                        while PCRE2.pcre2_jit_match_8(regex,
                          sequences, c_size_t(sequences_Length),
                          c_size_t(match.contents[1]), c_uint32(0), match_Data,
None)>=0:
                                match_Count+=1
                        ...
                        worker_Pipe.send((index_For_Pattern_To_Count, match_Coun
t))

                # If we got here, then this is a task requesting that replacemen
ts be
                # made to the sequences string using all the (pattern, replaceme
nt)
                # tuples in the task.
                else:
...
e_String to
                        # store the string after the replacements have been made
. After
                        # each iteration these two then get swapped. Make both s
trings 10%
                        # larger than the sequences string in order to accomodat
e some of
                        # the replacements causing sequences to grow and also co
py the
                        # sequences string into prereplace_String for the initia
l iteration.
                        ...
                        postreplace_String=create_string_buffer(string_Capacitie
s)
                        postreplace_String_Length=string_Capacities

                        # Iterate through all the (pattern, replacement) tuples
in the task.
                        for pattern, replacement in task:
                                postreplace_String, postreplace_String_Length=re
place(
                                  pattern, replacement,
                                  prereplace_String, prereplace_String_Length,
                                  postreplace_String, postreplace_String_Length)

                                # Swap prereplace_String and postreplace_String
in preparation
                                # for the next iteration.
                                prereplace_String, postreplace_String= \
                                  postreplace_String, prereplace_String
                                prereplace_String_Length=postreplace_String_Leng
th
                                postreplace_String_Length=string_Capacities
...
ry
        ...
        # Start a worker subprocess on each processor that is available to us an
d
        # send each worker subprocess the sequences string & a worker_Pipe to us
e
        ...
        # Now the manger process needs to start managing all the worker subproce
sses
        # by waiting for them to become ready to process tasks, handling any res
ults
...
irst element
                        # is the index of the pattern that the result is for and
 the second
                        # element is the number of matches for it. Add the numbe
r of matches
                        # to count_Info[].
                        elif type(result) is tuple:
                                count_Info[result[0]]=[count_Info[result[0]], re
sult[1]]
...
yet.
                        ...
                        # If we have reached the end of count_Info[] then there
are no more
                        # tasks to start working on so just send the worker subp
rocess None
                        # to indicate it can exit and also stop keeping track of
 the
                        # manger_Pipe for it.
                        else:
        ...
        # Print the size of the original input, the size of the input without th
e
        # sequence descriptions & new lines, and the size after having made all
the
        # replacements.
        print()
        print(input_Length)
