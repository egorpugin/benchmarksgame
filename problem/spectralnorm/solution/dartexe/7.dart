/* The Computer Language Benchmarks Game
   ...
   (start end ranges from Andrey Filatkinʼs JavaScript #6 program)
...
Float64List wOutReference = new Float64List(0);
final zeroList = TransferableTypedData.fromList([wOutReference]);
...
void main(List<String> args) async {
  ...
  mainIsolate.listen((dynamic m) {
    if (m is Message) {
      ...
      List.copyRange(wOutReference, m.start,
          m.data.materialize().asFloat64List(), m.start, m.end);
...
void requestReply(SendPort p) {
  ...
  requestPort.listen((dynamic m) {
    ...
    if (m is Message) {
      final data = m.data.materialize().asFloat64List();
      Float64List partialResult;
      if (m.shouldTranspose) {
        partialResult = atuFromTo(m.start, m.end, data);
      } else {
        partialResult = auFromTo(m.start, m.end, data);
      }
      m.data = TransferableTypedData.fromList([partialResult]);
      m.replyTo.send(m);
    }
  });
}
...
Future<void> mapToIsolates(
  ...
  startEndRanges.forEach((m) {
    ...
    m.data = TransferableTypedData.fromList([u]);
    final c = Completer<dynamic>();
    completers.add(c);
    barrier.add(c.future);
...
class Message {
  ...
  TransferableTypedData data = zeroList;
