    ...
    async function mainThread(n: number) {
        ...
        function startWorkers() {
            ...
            for (let i = 0; i < cpus; i++) {
                ...
                const worker = new NodeWorker(__filename, {workerData: {n, start
, end}});
        ...
        function stopWorkers() {
            workers.forEach(worker => worker.postMessage({ variant: MessageVaria
nt.Exit }));
        }
    }

    function workerThread({n, start, end}: {n: number, start: number, end: numbe
r}) {
            ...
            switch (message.variant) {
                case MessageVariant.Sab:
                    data = {
                        ...
                        w: new Float64Array(message.data, 2 * bytesPerFloat * n,
 n),
                    };
                    break;
