...
void reverse_Complement_And_Move_Span(uint8_t * dst_Front_Character
  , uint8_t * character_After_Src_Rear_Character, uintnative_t span_Length){
        ...
        // The loops farther down below will simultaneously reverse complement a
nd
        // move either sixteen or two characters so consequently they can only
        // properly handle even span_Lengths. If the span_Lengths are odd, then
this
        // if block below will do a single preliminary complement and move to ma
ke
        // the remaining span_Lengths an even number that the last loop can hand
le.
        if(span_Length%2){
                *dst_Front_Character++
...
,
        // then this loop can use it to simultaneously reverse complement and mo
ve a
        // sixteen character subspan at a time if the span_Length is at least th
at
        // large.
        #ifdef __SSE4_1__
                for(; span_Length>=16; span_Length-=16, dst_Front_Character+=16)
{
                        ...
                        // Load the subspan before character_After_Src_Rear_Char
acter.
...
;
                        ...
                        // AND all the characters in the subspan with 0b11111 wh
ich will
                        // cause the values for the ASCII letters A to Z AND a t
o z to
                        // change to 1 to 26. This allows a smaller lookup table
 to be used
                        // which is useful here because we are going to be using
 128 bit
...
t to use.
                        // Larger lookup tables would require using more registe
rs and doing
                        // more lookups which will result in slower code.
                        subspan=_mm_and_si128(subspan, _mm_set1_epi8(0b11111));


                        // Now we load two sixteen character lookup tables into
two SSE
                        // registers, lookup each character of the subspan in bo
th lookup
...
rite the
                        ...
                        //
                        // Note that if the below lookup tables are changed, the
n the
                        // COMPLEMENT_LOOKUP table up above should also be chang
ed. It would
                        // also be a good idea to read the comment above the
                        // COMPLEMENT_LOOKUP table for why there is a newline in
 this lookup
                        // table and why it is set as the complement of the lett
er J.
        ...
        // REVERSE_COMPLEMENT_LOOKUP table that was set up during startup to all
ow
        // us to reverse complement two characters at a time. This is faster tha
n
...
void * write_Reverse_Complements_Of_Chunks(
  void * initial_Semaphore_For_Signaling_Chunk_Written){
        ...
        // instead write the reverse complement of a smaller chunk to chunk_To_W
rite
        // and then we write chunk_To_Write after we finish reverse complementin
g
        // the chunk and all prior chunks have been written. By writing/reading
to/
        // from chunk_To_Write, which should be mostly cached, we avoid having t
o
        ...
        for(;;){
                ...
                pthread_mutex_lock(&shared_Variables_Mutex);
                        ...
                        // full_Line_Front_Span_Length is the number of nucleoba
ses that
                        // show up on the last line of the chunk (and also the s
equence).
                        // If the last line of the chunk is a non-full line, the
n
                        // full_Line_Rear_Span_Length is the number of nucleobas
es that
                        // would be required to be added to make it a full line,
 otherwise
                        ...
                        // Determine the chunk_Size we should use and the number
 of
                        // chunk_Characters_Left_To_Process. If there are no
                        // chunk_Characters_Left_To_Process, then exit the threa
d. Also
                        // update sequence_Characters_Left_To_Process so that th
e next
                        // thread looking for a chunk to work on knows the numbe
r of
                        // sequence_Characters_Left_To_Process.
                        uintnative_t chunk_Size
                          =sequence_Characters_Left_To_Process>LINES_PER_CHUNK*L
INE_LENGTH
...
;
                        ...
                        // Get a copy of the front_Character_Of_Last_Assigned_Ch
unk that we
                        // can use to keep track of the last_Processed_Chunk_Cha
racter. Also
                        // decrement front_Character_Of_Last_Assigned_Chunk by o
ur
                        // chunk_Size so that the next thread looking for a chun
k to work on
                        // will know to continue working from the front of our c
hunk.
                        ...
                        // Get a copy of semaphore_For_Signaling_Last_Assigned_C
hunk_Written
                        // so that we know what semaphore_To_Wait_On_Before_Writ
ing_Chunk.
                        // Also set semaphore_For_Signaling_Last_Assigned_Chunk_
Written to
                        // our semaphore_For_Signaling_Chunk_Written so that the
 next thread
                        // looking for a chunk to work on knows what semaphore i
t should
                ...
                // We now have all information we need to know and can start rev
erse
                // complementing our chunk.

                // First we check to see if we have an optimal size sequence tha
t
                // entirely consists of full lines (which will be the case when
                // full_Line_Rear_Span_Length is zero). In this case we can just
 make a
                // single call to reverse_Complement_And_Move_Span() and have it
 reverse
                ...
                // This loop reverse complements and moves full lines and also k
nows how
                ...
                for(; chunk_Characters_Left_To_Process>=LINE_LENGTH
                  ; chunk_Characters_Left_To_Process-=LINE_LENGTH){
                        ...
                        // Reverse complement and move a full_Line_Front_Span_Le
ngth of
                        // nucleobases ending in front of the last_Processed_Chu
nk_Character
                        // to chunk_Pos (which will now be pointing at the posti
on for the
                        // first nucleobase of a line). Then chunk_Pos and
                        // last_Processed_Chunk_Character are incremented/decrem
ented
                        // respectively by full_Line_Front_Span_Length and
                        // last_Processed_Chunk_Character is decremented by one
more to skip
                        ...
                        last_Processed_Chunk_Character-=full_Line_Front_Span_Len
gth+1;

                        // Reverse complement and move a full_Line_Rear_Span_Len
gth of
                        // nucleobases ending in front of the last_Processed_Chu
nk_Character
                        // (which will now be pointing at the newline after the
last
                        // nucleobase of a line) to chunk_Pos. Then chunk_Pos an
d
                        // last_Processed_Chunk_Character are incremented/decrem
ented
                        ...
                        last_Processed_Chunk_Character-=full_Line_Rear_Span_Leng
th;
...
and the
                // line will contain a full_Line_Front_Span_Length of nucelobase
s plus a
                // preceding newline. Reverse complement and move that partial l
ine here
                // if necessary.
                if(chunk_Characters_Left_To_Process)
                        reverse_Complement_And_Move_Span(chunk_Pos
                ...
                // Before we can write the chunk_To_Write that we just finished
reverse
                // complementing, we need to wait for a signal from the
                // semaphore_To_Wait_On_Before_Writing_Chunk so that we know the
                // previous chunk (and all prior chunks) have been written first
. Once
                // that happens we can then write the chunk_To_Write. We then si
gnal the
                // semaphore_For_Signaling_Chunk_Written so that the thread proc
essing
                // the next chunk (if any) knows that it is OK for it to start w
riting.
...
line
        ...
        // front_Character_Of_Last_Assigned_Chunk is initially set to point to t
he
        // last newline of the sequence (if the sequence was one chunk larger, t
his
        ...
        // Figure out the number_Of_Threads_To_Use based on how many CPU cores a
re
        // available. Note that using sysconf(_SC_NPROCESSORS_ONLN) will return
the
        ...
        // Initialize all the chunk_Written_Semaphores. Although no chunks have
been
        // written yet, the thread processing the first chunk (if any) will expe
ct
        // to see a chunk_Written_Semaphore signaling that all prior chunks have
        // been written. To meet its expectations we pretend that a prior chunk
has
        // already been written by assigning the last chunk_Written_Semaphores t
o
        // semaphore_For_Signaling_Last_Assigned_Chunk_Written and initializing
it
        // as already being signaled.
        for(uintnative_t i=number_Of_Threads_To_Use+1; i-->0;)
                sem_init(&chunk_Written_Semaphores[i], 0, i/number_Of_Threads_To
_Use);
        semaphore_For_Signaling_Last_Assigned_Chunk_Written
          =&chunk_Written_Semaphores[number_Of_Threads_To_Use];

        // Create number_Of_Threads_To_Use chunks_Processing_Threads and have th
em
        ...
        for(uintnative_t i=number_Of_Threads_To_Use; i-->0;)
                pthread_create(&chunks_Processing_Threads[i], NULL
                  , write_Reverse_Complements_Of_Chunks, &chunk_Written_Semaphor
es[i]);

        // Wait for all the chunks_Processing_Threads to finish writing the reve
rse
...
int main(void){
        ...
        // We initialize the REVERSE_COMPLEMENT_LOOKUP table here but only fill
in
        ...
        // Read in sequence data until we reach the end of the file or encounter
 an
        // error.
...
he ʼ>ʼ that
                        ...
                        // If there is any data for the current sequence, then p
rocess it
...
t to the front
                                ...
                                // Reset sequence_Size to 0 as we start processi
ng the next
                                // sequence.
                                sequence_Size=0;
                        }
