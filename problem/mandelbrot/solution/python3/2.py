    ...
    while 1:
        try:
            ...
            if len(buff):
                ...
                pos = len(buff) - 1
                while pos>0:
                    if buff[pos][0] == buff[pos-1][1]:  #connect contiguous segm
ent
                        buff[pos-1] = (buff[pos-1][0],buff[pos][1],buff[pos-1][2
]+buff[pos][2])
                        del(buff[pos])
                    pos-=1
...
def worker(size, task_queue, buff_queue):
    ...
    while 1:
        ...
        if task is None:
            ...
            if len(result):      # try to resume from previous segment unless br
oken
                buff_queue.put((task_head,y+1,result))
            task_head = task
            result = []
