...
namespace {
    ...
    bool vec_is_any_le(Vec v, Vec f) { return bool(_mm512_cmp_pd_mask(v, f, _CMP
_LE_OS)); }
    int vec_is_le(Vec v1, Vec v2)    { return _mm512_cmp_pd_mask(v1, v2, _CMP_LE
_OS); }
...
0x3F, 0xBF, 0x7F, 0xFF
    ...
    bool vec_is_any_le(Vec v, Vec f) { Vec m = v<=f; return ! _mm256_testz_pd(m,
 m); }
    ...
    bool vec_is_any_le(Vec v, Vec f) { __m128i m = __m128i(v<=f); return ! _mm_t
estz_si128(m, m); }
    ...
    // up computations.
    //
    void calcSum(Vec* real, Vec* imag, Vec* sum, const Vec* init_real, Vec init_
imag)
    {
